      ...
      <el-row justify="space-between" type="flex" :gutter="20" class="twoRow">
        <el-col :span="24">
          <el-button-group>
            ...
            <!-- 导入 -->
            <el-button class="el-button_co" type="primary" @click="doUploadImgs" icon="el-icon-upload2">导入</el-button>
            <!--批量下发-->
            <el-button class="el-button_co" @click="doDownImg" type="primary" icon="el-icon-download">批量下发</el-button>
            <!-- 导出 -->
            <el-popconfirm
      ...
      <el-row>
        ...
        >
          ...
          <el-table-column label="签到时间" align="center">
            <template scope="scope">{{ scope.row.signIn_time.substr(0, 16) }}</template>
          </el-table-column>
    ...
    >
      ...
      >
        <el-row style="margin-bottom: -15px">
          <el-col :span="15">
            <el-form-item label="用户ID" prop="user_id" autocomplete="off" v-show="false">
              <el-input v-model="meetingManage.meeting_id"></el-input>
            </el-form-item>
            <el-form-item label="会议名" prop="username" autocomplete="off">
              <input
                name="u"
                type="text"
                placeholder="会议名"
              ...
              <el-input
                ...
                size="small"
                placeholder="请输入会议名"
                v-model="meetingManage.meeting_name"
                :maxlength="12"
                :readonly="readOnlyFlag"
                @focus="readOnlyFlag = false;"
                autocomplete="new-accounts"
      ...
      >
        <el-row style="margin-bottom: -15px">
          <el-col :span="9">
            <el-form-item v-if="this.configGd.isAdmin" label="所属企业" prop="company_name">
              <el-select
                v-model="importForm.company_name"
                clearable
                size="small"
                style="width:200px;"
                placeholder="请选择所属企业"
        ...
        <el-form-item style="padding-left: 135px; margin: -10px 0 5px 0">
          <el-button
            ref="submitBut"
            type="primary"
            @click="plimport('importForm')"
            size="small"
            icon="el-icon-circle-check"
            :loading="diaLoading"
          >{{butTitleSubmit}}
          </el-button>
          <el-button @click="resetForm('importForm')" size="small" icon="el-icon-refresh-left">重置</el-button>
        </el-form-item>
      </el-form>
    </el-dialog>
  ...
  export default {
    data() {
      return {
        ...
        importdialogVisible: false, //导入用户信息对话框
        meetingmanages:false, //添加会议对话框
        UploadUrl: 'http://localhost:8086/',
        delicoVisible: false,
        imageUrl: '',
        butTitleSubmit: '立即添加',
    ...
    methods: {
      ...
      apiInportImg() {
        ...
        if (
          !this.importForm.company_name ||
          this.importForm.company_name.length === 0
        ) {
          this.importForm.company_name = this.$Storage.localGet('companyName')
        }
        const formData = new FormData()
        formData.append("companyName", this.importForm.company_name)
        if (this.fileList && this.fileList.length > 0) {
      ...
      toAddusers() {
        this.isATyle = 1
        this.adTitlem = '添加会议信息'
        this.intSimData()
        this.udShowStyle(false)
        this.butTitleSubmit = '立即添加'
        this.placeholder_pass = '请输入登录密码'
        this.meetingmanages = true
      },
      ...
      async doDownImg(scope) {
        if (scope && scope.row) {
        } else if (!this.selId.length) {
          this.$message({message: '请选择您需要下发的人员!', type: 'warning'})
          return false
        }
        var mySelList = this.getSelList(scope.row)
        if (mySelList) {
          ...
          const confirmResult = await this.$confirm(
            '您确定要下发 [' + mySelList.SelNamesStr + ']' + myNumsTip + ' 吗?',
            '下发人员信息',
            {
              cancelButtonText: '取消',
          ...
          if (confirmResult !== 'confirm') {
            return this.$message.info('已取消下发')
          }
          this.udShowStyle(true)
          this.$http
            .post('/api' + '/user/issueStaff', mySelList.SelIDs)
            .then(res => {
              // console.log(res)
              if (res.data.code === 0) {
                this.dialogVisible = false
                ...
                this.urResponseErr('下发人员信息失败：' + res.data.message)
              }
            })
            // eslint-disable-next-line handle-callback-err
            .catch(err => {
              this.urResponseErr('下发人员信息失败！')
            })
        }
